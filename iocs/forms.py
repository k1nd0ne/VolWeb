...
class IOCForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'name': TextInput(
                attrs={'class': 'form-control', 'placeholder': 'Usually the threat name', 'required': '""'}),
            'value': Textarea(
                attrs={"class": "form-control", "rows": "4", "required": "", 'placeholder': 'What to look for'}),
            'context': TextInput(
                attrs={'class': 'form-control', 'rows': "4", 'placeholder': 'Context to qualify the IOC',
                       'required': '""'}),
