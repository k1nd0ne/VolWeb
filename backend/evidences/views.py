...
class BindEvidenceViewSet(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        required_fields = ['os', 'linked_case', 'source', 'access_key_id', 'access_key', 'url', 'endpoint']
            ...
            if parsed_url.scheme == 's3':
                ...
                if 's3' in netloc_parts:
                    ...
                    if len(path_parts) == 2:
                        ...
                        raise ValueError('Invalid URL format. Cannot extract bucket and key.')
            else:
        ...
        try:
            ...
            return Response({'detail': f'Error parsing URL: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if source == 'AWS':
            ...
            try:
                ...
                if error_code == '404':
                    return Response({'detail': 'Object does not exist in S3.'}, status=status.HTTP_404_NOT_FOUND)
                else:
                    return Response({'detail': f'AWS ClientError: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            try:
                ...
                return Response({'detail': f'Error accessing MinIO: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
        else:
            return Response({'detail': 'Invalid source specified. Must be AWS or MINIO.'}, status=status.HTTP_400_BAD_REQUEST)
