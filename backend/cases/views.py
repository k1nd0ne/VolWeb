        ...
        if serializer.is_valid():
            ...
            try:
                upload_session = UploadSession.objects.get(
                    upload_id=upload_id, user=request.user
                )
            except UploadSession.DoesNotExist:
                return Response(
                    {"error": "Invalid upload_id or unauthorized access."},
            ...
            if not os.path.exists(temp_dir):
                return Response(
                    {"error": "Upload session has expired or is invalid."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            try:
                ...
                return Response(
                    {"error": "Invalid chunk filenames."},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            final_filename = upload_session.filename
            final_file_path = os.path.join(
                settings.MEDIA_ROOT, "evidences", final_filename
            )
            ...
            return Response(
                {"status": "upload complete", "evidence_id": evidence.id},
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response(
                    {"error": "Invalid case_id."}, status=status.HTTP_400_BAD_REQUEST
                )
