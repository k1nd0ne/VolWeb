function generate_tag(plugin_name, item) {
  ...
  dropdown_menu.setAttribute(
    "class",
    "dropdown-menu shadow dropdown-menu-end animated--fade-in"
  );
  ...
  badge_suspicious.setAttribute(
    "onclick",
    "Tag('" + plugin_name + "'," + item.id + ", 'Suspicious');"
  );
  ...
  badge_evidence.setAttribute(
    "onclick",
    "Tag('" + plugin_name + "'," + item.id + ", 'Evidence');"
  );
  ...
  badge_clear.setAttribute(
    "onclick",
    "Tag('" + plugin_name + "'," + item.id + ", 'Clear');"
  );
  ...
  if (item.Tag == "Evidence") {
    tag_evidence.setAttribute(
      "class",
      "badge bg-danger text-wrap tag_evidence_" + item.id + "_" + plugin_name
    );
    tag_suspicious.setAttribute(
      "class",
      "badge bg-warning text-wrap d-none tag_suspicious_" +
        item.id +
        "_" +
        plugin_name
    );
    ...
    tag_evidence.setAttribute(
      "class",
      "badge bg-danger text-wrap d-none tag_evidence_" +
        item.id +
        "_" +
        plugin_name
    );
    tag_suspicious.setAttribute(
      "class",
      "badge bg-warning text-wrap tag_suspicious_" + item.id + "_" + plugin_name
    );
  } else {
    tag_evidence.setAttribute(
      "class",
...
function Tag(plugin_name, artifact_id, tag) {
  ...
  $.ajax({
    ...
    success: function (response) {
      if (tag == "Evidence") {
        $(".tag_evidence_" + artifact_id + "_" + plugin_name).removeClass(
          "d-none"
        );
        $(".tag_suspicious_" + artifact_id + "_" + plugin_name).addClass(
          "d-none"
        );
      }
      if (tag == "Suspicious") {
        $(".tag_suspicious_" + artifact_id + "_" + plugin_name).removeClass(
          "d-none"
        );
        $(".tag_evidence_" + artifact_id + "_" + plugin_name).addClass(
          "d-none"
        );
      }
      if (tag == "Clear") {
